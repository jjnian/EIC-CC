...
| **隐式引用补全** | 生产库普遍不建外键——按命名约定（`xxx_id`/`xxxId`/`no`/`code`，支持复数/驼峰/表前缀）推断无 FK 声明的引用血缘，`inferred/0.5` 候选直出，可被值包含检验一键佐证升级、自动进审核队列 |
...
9. **库内结构血缘**：DDL 导出时，`SchemaGraphFragmentRenderer` 把库内血缘渲染成结构化片段随经验直连合并（免 LLM 重抽）：① 外键 → `depends_on` 边（`derived/1.0`，父表=上游）；② 视图定义（FROM/JOIN 来源表 → 视图）→ `flows_to` 边（`derived/1.0`）；③ 存储过程/函数源码（INSERT…SELECT / MERGE / UPDATE…FROM 的来源 → 写入目标）→ `flows_to` 边（`derived/1.0`，`SchemaSqlLineage` 语句级解析）；④ **隐式引用**（`ImplicitRefInferencer` 命名约定推断，生产库常无 FK）→ `depends_on` 边（`inferred/0.5` 候选，证据格式兼容值包含检验，可一键佐证/进审核队列）。存储过程源码节选同时附在 DDL 文档里供 LLM 做业务语义增强。
10. **图校验 + 证据聚合**（`sanitizeGraph`）：丢弃空/重复 id 节点、悬空边、自环；按 (from,to,rel_type) 去重时**不丢证据**——重复边往往是不同来源各说了一遍，其证据聚合进保留边的 `evidences` 列表，并按独立证据数提升置信度（多源佐证封顶 0.9，确定性 1.0 不动，来源取更强者）。前端「合并进模型」对已有边重复时走同一套聚合规则（`evidenceMerge.ts`）。
...
### 7.4 图谱可视化与编辑
自绘 SVG 画布：节点拖拽、框选、缩放平移、右键菜单、批量操作；节点 / 边增删改；节点详情与 Schema（属性 / 约束）编辑；撤销 / 重做历史；图谱搜索定位；节点着色（按类型 / 领域）；导入合并与去重；**领域聚合折叠**（`useDomainCollapse`：把整个业务域折叠成一个超级节点，域内边隐藏、跨域边聚合为 ×N，点击超级节点展开——大图复杂而不混乱，纯渲染层派生、不改真实图）。

### 7.5 血缘分析与体检（分析面板 6 个标签）
- **全图统计**：规模、类型分布、度分布等。
...
- **覆盖度**：把经验库（文件夹=领域）与图对照——每域的节点/边规模、经验来源分布、推断占比、孤立聚集；规则化找缺口（无经验支撑域 / 单一来源域 / 推断占比过高 / 证据覆盖低），生成**调研任务清单**并可导出《覆盖度审计报告》——完整性靠「收集→建图→找缺口→再收集」迭代。
- **审核**：低置信 / 孤证 / 无证据的推断边排队给业务专家把关——**确认**（`review_status=confirmed`、置信度提至 0.9、可附批注）或**否决**（删边，可撤销/有版本快照）；多源佐证（🔗×N）与审核状态在边详情可见。
...
| 隐式引用推断 | 无 FK 声明的 `xxx_id` 形态列按命名约定推断为 `inferred/0.5` 的 `depends_on` 候选边，佐证后升级 |
