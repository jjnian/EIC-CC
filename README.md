...
| 入口 | 行为 |
|---|---|
| 顶栏「预览」 | 在 **新浏览器标签页** 以**只读全屏**模式打开当前图谱：无侧栏、无对话框、无编辑按钮，仅支持 pan / zoom / 选中查看节点详情 / 适应屏幕 / 切 Schema / 导出 JSON。URL 形如 `?preview=om_xxx`，可以直接分享给他人（前提是 ta 能访问后端） |
| 顶栏「导出」 | 下载当前图谱为 JSON（含元信息、所有节点 / 边、导出时间戳） |
| 顶栏「共享」 | 把图谱摘要（标题 + 节点数 + 前 10 个节点）写入剪贴板 |

> 「预览」按钮在点击之前会先 flush 一次防抖保存，避免新标签页加载到旧数据。
...
### 6.7 推演深化能力（概率约束 / 原始 prompt 查看 / 节点详细解释）

以下三个进阶能力的完整使用说明见独立文档 [`PREDICTION-DEEPENING-GUIDE.md`](PREDICTION-DEEPENING-GUIDE.md)：

| 能力 | 入口 | 一句话作用 |
|---|---|---|
| **概率约束** | PredictDialog → What-if 约束 → 紫色「概率」按钮 | 给节点设 0~1 先验，作为下游联合概率入口初值 |
| **原始 prompt 查看** | 推演消息头部 `📜 prompt` 按钮 | 看本次推演发给 LLM 的完整 system + user 文本 |
| **"为什么会发生？"** | 推演节点右键 → 🔍 为什么会发生？ | LLM 生成三段式解释（依据/假设/反例），结果缓存到分支文件 |
