...
| **确定性血缘** | 数据库外键（FK）作为引用血缘的 ground truth，确定性直出为 `confidence=1.0` 的血缘边，不靠 LLM 猜 |
...
9. **外键确定性血缘**：DDL 导出时，`SchemaGraphFragmentRenderer` 把数据库外键渲染成结构化片段（表→`entity` 节点、FK→`depends_on` 边，`source=derived`、`confidence=1.0`，父表=上游、子表=下游），随经验直连合并 —— 库表引用血缘保证正确，不靠 LLM 猜。
...
- **数据库结构导出**：内省 schema → 渲染 DDL（含样例数据可选）→ 存为经验，并内嵌外键血缘结构化片段。
- **系统探索**：见 7.7。
- **联网调研**：见 7.8。
