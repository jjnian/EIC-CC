...
public final class ExplorePrompts {
    ...
    public static final String EXPLORE_AGENT_SYSTEM = """
        ...
        探索策略(很重要):
        1. **广度优先、系统化、要全**:把**每一个**导航菜单项 / 顶级模块 / 二级菜单都点一遍,优先点**没去过的**,
           把整棵功能树铺开;不要只看一两个模块就觉得"了解了"。系统的每个功能页都值得进去看一眼。
        2. **别重复但别漏**:已访问过的页面(看进度列表)不要再进;但只要还有没去过的菜单/模块入口,就继续点,
           不要轻易 done。只有当**所有**可见入口都访问过、确实没有新页面时,才考虑结束。
